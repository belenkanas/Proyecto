...
public class JugadorPrincipal : IJugador
{
    ...
    public RestriccionNombrePokemon RNombrePokemon{ get; set; }
    public RestriccionSoloTipoPokemon RSoloTipo { get; set; }
    public RestriccionNoTipoDePokemon RNoTipo { get; set; }
    public RestriccionItem RItem { get; set; }
    
    public List<IRestriccion> Restricciones { get; set; }
    ...
    public JugadorPrincipal(string nombre)
    {
        ...
        RNombrePokemon = new RestriccionNombrePokemon();
        RSoloTipo = new RestriccionSoloTipoPokemon();
        RNoTipo = new RestriccionNoTipoDePokemon();
        RItem = new RestriccionItem();
        Restricciones = new List<IRestriccion>{RNombrePokemon,RSoloTipo,RNoTipo,RItem};
    }
    ...
    {
        var msg = PokemonActual.UsarAtaque(indiceAtaque, enemigo,
            this); // Supongamos que elegimos un ataque del Pokémon
        var a = pokemon;
        var b = enemigo;
    ...
    public string MostrarAtaquesDisponibles(int indice)
    {
        ...
        if (indice >= 0 && indice < EquipoPokemons.Count)
        {
            ...
            {
                cadena +=
                    $"{i + 1}. Nombre: {pokemon.Ataques[i].Nombre} Tipo: {pokemon.Ataques[i].TipoAtaque.NombreTipo} Daño: {pokemon.Ataques[i].DañoBase} Es especial: {pokemon.Ataques[i].EsEspecial}.\n";
            }
        }
        else
        {
    ...
    public string MostrarRestricciones()
    {
        ...
        int indice = 0;
        foreach (IRestriccion restriccion in Restricciones)
        {
            restricciones += $"{indice}- {restriccion.NombreRestriccion} \n";
            indice++;
        }
    ...
    public string ElegirRestriccion(int indiceRestriccion, string nombre)
    {
